...
public class Tools {
    ...
    public static String getDateOfAfter(float day){
        Date date = new Date();
        date.setTime(date.getTime()+(int)(day*(24*60*60*1000)));
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        dateFormat.setTimeZone(TimeZone.getTimeZone("Asia/Shanghai"));
        return dateFormat.format(date);
    }
    public static String getUTF(String src) throws UnsupportedEncodingException{
    ...
    public static String getDateAddDay(String date, float day) {
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        dateFormat.setTimeZone(TimeZone.getTimeZone("Asia/Shanghai"));
        Date newDate=null;
        try {
            newDate = dateFormat.parse(date);
            newDate.setTime(newDate.getTime()+(int)(day*(24*60*60*1000)));
