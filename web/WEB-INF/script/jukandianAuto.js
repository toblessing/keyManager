...
var appName = "聚看点"
var plateClassName = "android.widget.LinearLayout"
var plateDepth = 3
var delay;
...
while (true) {
    ...
    var plateList = className(plateClassName).find()
    var isZero = true
    if (plateList != null) {
        //脚本循环主体
        plateList.forEach(function (onePlate) {
            ...
            if (exceptionPlateCheck(onePlate)) {
                ...
                if (onePlate.findOne(id("item_artical_three_title_tv")) != null) {
                    title = onePlate.findOne(id("item_artical_three_title_tv"))
                } else {
                    title = onePlate.findOne(id("item_artical_right_title_tv"))
                }
...
function clickHome() {
    toast("点击头条")
    var home = text("刷新").depth(2).findOne(5000)
    home.parent().click()
...
function exceptionPlateCheck(onePlate) {
    ...
    if (onePlate.id() != "com.xiangzi.jukandian:id/item_artical_three_parent" && onePlate.id() != "com.xiangzi.jukandian:id/item_artical_right_parent") {
        toast(onePlate.id())
    } else if (onePlate.findOne(textMatches("广告.*"))) {
...
function exceptionWindowDispose() {
    ...
    } else if (id("close_button").findOne(10) != null) {
        id("close_button").findOne(10).click()
    }
