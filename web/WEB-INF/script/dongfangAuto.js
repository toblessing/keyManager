...
while (true) {
    ...
    var plateList = className("android.widget.LinearLayout").find()
    var isZero = true
    if (plateList != null) {
        //脚本循环主体
...
function clickHome() {
    toast("点击头条")
    var home = text("新闻").findOne(1000)
    if (home == null) {
        home = text("刷新").findOne(1000)
    }
    home.parent().click()
}
...
function exceptionPlateCheck(onePlate) {
    ...
    } else if (onePlate.findOne(textMatches("\\d+万?阅读")) == null) {
...
function exceptionWindowDispose() {
    ...
    } else if (text("立即领取").findOne(10) != null) {
        text("立即领取").findOne(10).click()
        sleep(1000)
        if (text("忽略").findOne(10)) {
            text("忽略").findOne(10).click()
        } else {
            back()
        }
