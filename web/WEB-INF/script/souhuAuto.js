...
while (true) {
    ...
    if (plateList != null) {
        ...
        plateList.forEach(function (onePlate) {
            ...
            //筛选出文章板块
            if (exceptionPlateCheck(onePlate) && onePlate.findOne(id("article_title")) !== null) {
...
function exceptionPlateCheck(onePlate) {
    ...
    } else if (onePlate.findOne(id("article_title").drawingOrder(2)) == null) {
    } else if (onePlate.findOne(text("淘宝精选"))) {
    } else if (onePlate.findOne(text("视频"))) {
    } else if (onePlate.findOne(text("查看更多"))) {
    } else if (onePlate.findOne(text("热点"))) {
    } else if (onePlate.findOne(textMatches(/[\u4e00-\u9fa5|，,.。：:？?<>《》“”""！!|0-9]{7,}/)) == null) {
    } else if (onePlate.findOne(textMatches("\\d+图"))) {
    } else if (onePlate.findOne(textMatches("\\d+:\\d+"))) {
    } else {
        return true
    }
